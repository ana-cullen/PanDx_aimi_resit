from SaBN.nnUNetTrainerSaBN import nnUNetTrainerSaBN
# from SaBN import nnUNetTrainerSaBN
...
class nnUNetTrainerCELossLesionSplitSaBN(nnUNetTrainerSaBN):
    def _build_loss(self):
        assert not self.label_manager.has_regions, 'regions not supported by this trainer'
        loss = RobustCrossEntropyLoss(weight=None,
                                      ignore_index=self.label_manager.ignore_label if self.label_manager.has_ignore_label else -100)
...
# class nnUNetTrainerCELossLesionSplitBN(nnUNetTrainerBN):
#     def _build_loss(self):
#         assert not self.label_manager.has_regions, 'regions not supported by this trainer'
#         loss = RobustCrossEntropyLoss(weight=None,
#                                       ignore_index=self.label_manager.ignore_label if self.label_manager.has_ignore_label else -100)
